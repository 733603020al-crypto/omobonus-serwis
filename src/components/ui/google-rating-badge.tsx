...
export function GoogleRatingBadge({ className = '' }: { className?: string } = {}) {
  ...
  const shell =
    `flex items-center gap-3 rounded-lg border-2 border-[rgba(200,169,107,0.5)] hover:border-[rgba(200,169,107,0.85)] bg-black/45 px-4 py-2 backdrop-blur-[2px] transition-colors ${className}`
    ...
    >
      ...
      <div className="flex flex-col leading-none text-left">
        ...
        <div className="flex items-center gap-1.5 mt-0.5">
          <span className="font-sans text-[26px] font-bold text-[#bfa76a]">
            {rating.toFixed(1)}
          </span>
          <span className="text-[#f3df9a] text-[22px] leading-none tracking-tight">
            {'★★★★★'}
          </span>
        </div>
        <span className="mt-0.5 text-[12px] text-white/75 font-sans tracking-wide">
          Zaufanie klientów
        </span>
      </div>
    </a>
