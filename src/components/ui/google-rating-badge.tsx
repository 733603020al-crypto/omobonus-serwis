...
export function GoogleRatingBadge({ className = '' }: { className?: string } = {}) {
  ...
  const shell =
    `relative group flex items-center gap-3 rounded-lg border-2 border-[#bfa76a]/80 bg-[#bfa76a]/10 shadow-[0_0_20px_rgba(191,167,106,0.35)] px-4 py-2 backdrop-blur-[2px] transition-all duration-[180ms] hover:-translate-y-1 hover:bg-[#bfa76a]/20 hover:shadow-[0_0_28px_rgba(191,167,106,0.45)] cursor-pointer ${className}`
  ...
  return (
    ...
    >
      ...
      <svg width="37" height="37" viewBox="0 0 48 48" className="shrink-0">
      ...
      <div className="flex flex-col leading-none text-left">
        <span className="text-[12px] text-[#bfa76a] font-sans font-normal tracking-normal">
          Google Rating
        </span>
        <div className="flex items-center gap-1 mt-1.5">
          <span className="font-sans text-[24px] font-bold text-[#bfa76a]">
            {rating.toFixed(1)}
          </span>
          <span className="text-[#f3df9a] text-[18px] leading-none tracking-tight">
            {'★★★★★'}
          </span>
        </div>
        <span className="mt-1.5 text-[12px] text-white/75 font-sans tracking-wide">
          Zaufanie klientów
        </span>
      </div>
    </a>
