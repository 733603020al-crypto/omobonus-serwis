...
Ответ содержит только статус ("ok" с кодом 200 или "error" с кодом 503, если SMTP не настроен) и время. Имена отсутствующих переменных наружу не отдаются — их смотрите в настройках Vercel и в логах функции `/api/send-email`.
